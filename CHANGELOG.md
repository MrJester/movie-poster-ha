...
- Split each browser's responsive device matrix into four measured,
  load-balanced partitions and its remaining acceptance tests into two
  single-worker shards so long WebKit runs remain within hosted-runner limits.
...
- Add component duplication and reset, phone/tablet/television/signage/
